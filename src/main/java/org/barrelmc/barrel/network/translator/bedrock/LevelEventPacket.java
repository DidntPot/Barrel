...
public class LevelEventPacket implements BedrockPacketTranslator {
    ...
    public void translate(BedrockPacket pk, Player player) {
        ...
        if (packet.getType() == LevelEventType.BLOCK_START_BREAK) {
            ...
            player.javaSession.send(new ClientboundBlockDestructionPacket(0, new Position((int) pos.getX(), (int) pos.getY(), (int) pos.getZ()), BlockBreakStage.STAGE_1));
        } else if (packet.getType() == LevelEventType.BLOCK_STOP_BREAK) {
            Vector3f pos = packet.getPosition();
            player.javaSession.send(new ClientboundBlockDestructionPacket(0, new Position((int) pos.getX(), (int) pos.getY(), (int) pos.getZ()), BlockBreakStage.RESET));
        }
    }
}
