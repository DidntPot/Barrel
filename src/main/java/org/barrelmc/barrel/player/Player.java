...
public class Player extends Vector3 {
    ...
    public final Session javaSession;
    public BedrockClient bedrockClient;
    public final PacketTranslatorManager packetTranslatorManager;
    ...
    public Player(ServerboundHelloPacket loginPacket, Session javaSession) {
        ...
        if(AuthManager.getInstance().band.get(this.username) == "Geyser"){
            ClientboundDisconnectPacket clientboundDisconnectPacket = new ClientboundDisconnectPacket("Pls join using Bedrock ip");
            this.javaSession.send(clientboundDisconnectPacket);
        }
        if (ProxyServer.instance.config.auth.equals("offline")) {
            this.offlineLogin(loginPacket);
            System.out.println("wat1");
        } else {
            this.accessToken = AuthManager.getInstance().accessTokens.remove(loginPacket.getUsername());
    ...
    private void onlineLogin(ServerboundHelloPacket javaLoginPacket) {
        ...
        ProxyServer.instance.onlinePlayers.put(javaLoginPacket.getUsername(), this);

        client.bind().join();

        Config config = ProxyServer.instance.config;
        InetSocketAddress bedrockAddress = new InetSocketAddress(config.bedrockAddress, config.bedrockPort);
        client.connect(bedrockAddress).whenComplete((session, throwable) -> {
            ...
            session.setPacketCodec(ProxyServer.instance.bedrockPacketCodec);
    ...
    public LoginPacket getOnlineLoginPacket() throws Exception {
        ...
        loginPacket.setProtocolVersion(ProxyServer.instance.bedrockPacketCodec.getProtocolVersion());
        return loginPacket;
    }
    ...
    private String getSkinData() {
        ...
        skinData.put("DeviceModel", "Barrel Dev");
        skinData.put("DeviceOS", 7);
        skinData.put("GameVersion", ProxyServer.instance.bedrockPacketCodec.getMinecraftVersion());
        ...
        skinData.put("ServerAddress", ProxyServer.instance.config.bedrockAddress + ":" + ProxyServer.instance.config.bedrockPort);
        skinData.put("SkinAnimationData", "");
        skinData.put("SkinColor", "#0");
        skinData.put("SkinGeometryDataEngineVersion", Base64.getEncoder().encodeToString("1.18.10".getBytes()));
    ...
    public void sendMessage(String message) {
        this.javaSession.send(new ClientboundChatPacket(Component.text(message)));
    }

    public void sendAlert(String message) {
        this.javaSession.send(new ClientboundChatPacket(Component.text("§f[§l§bBa§9rr§bel§f§r]"+message)));
    }

    public void sendTip(String message) {
        this.javaSession.send(new ClientboundChatPacket(Component.text(message), MessageType.NOTIFICATION));
    }

    public boolean usePlayerAuthInputPacket(){
        return !movementMode.equals(AuthoritativeMovementMode.CLIENT);
    }

    public void disconnect(String reason) {
        this.bedrockClient.getSession().disconnect();
        this.javaSession.disconnect(reason);
        ProxyServer.instance.onlinePlayers.remove(username);
    }
}
